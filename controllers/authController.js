...
const login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.rows[0].id, nombre: user.rows[0].nombre, apellidos: user.rows[0].apellidos, rol: user.rows[0].rol },
      process.env.JWT_SECRET,
      { expiresIn: '1h' }
    );

    res.json({ message: 'Inicio de sesión exitoso', token, rol: user.rows[0].rol });
